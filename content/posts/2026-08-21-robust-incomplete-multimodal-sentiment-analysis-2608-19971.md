...
* 技术严谨性 (1.0/1.5)：[A_RESULTS] 实验协议描述完整、消融变量隔离清楚；扣分在于基线数字沿用他文报告值而非统一重跑，且全文无种子方差报告，小幅度改进的可信度打折。基线数字沿用他文报告值而非统一重跑是最主要的严谨性折扣。
...
* 工程/实践价值 (0.6/1.5)：[A_ENGINEERING] 随机缺失下的稳健性对真实部署有吸引力，可靠性加权的设计可直接借鉴；但未报告推理延迟与算力成本。可靠性加权融合的设计可直接移植到其他残缺多模态管线。
...
8. 审稿人发现：代理质量随迭代轮数的变化缺乏直接度量，框架声称的「逐步精炼」没有被可视化或量化验证。
9. 审稿人发现：三个基准均为实验室采集的片段级数据，真实流媒体场景中的长视频、多说话人与背景音乐干扰未被覆盖。
