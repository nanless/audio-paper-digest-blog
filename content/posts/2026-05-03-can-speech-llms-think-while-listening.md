...
本文基于**Moshi**多流语音大模型架构进行扩展。Moshi是一个全双工模型，同时处理三个流：用户音频（AU）、系统音频（AS）和系统文本（文本独白TS）。所有流在时间上对齐。

核心修改在于对**文本独白（TS）流**的利用和扩展，使其不仅承载系统语音对应的文本，还承载**流式用户ASR转写文本（QT）** 和**推理文本（RT）**。
...
**模型架构与流交互示意图**：
![模型架构与流交互](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/dFVenZdVbX-0.png)
...
- **训练数据**：主要使用**CoT-Collection**数据集（1.8M样本）。经过处理：1）过滤长问题（>60词）至约69万；2）使用LLM进行“语音友好化”重写（问题、推理、答案）；3）使用内部TTS引擎（24kHz单声道）将问答对转为语音。训练时，文本CoT直接使用重写后的文本。
- **损失函数**：监督微调（SFT）使用标准的**负对数似然（NLL）损失**，用于下一个token预测。偏好优化阶段使用**DPO损失**，并引入长度归一化和在偏好数据`yw`上的NLL损失以稳定训练，总损失为 `Lpref = LDPO - λE[log πΘ(yw|x)]`，其中λ=0.1，β=0.1。
- **训练策略**：
...
- **推理细节**：使用**强制解码（force-decoding）**。在用户问题开始时强制注入k个`[PAD]`以适应流式ASR。如果模型未生成`<start_cot>`，则在问题结束后强制注入该令牌以触发CoT生成。延迟测量通过Pyannote VAD检测响应音频的起始点计算，并折算为token数（1 token ≈ 80ms）。

### 📊 实验结果

论文构建了**语音推理问答（SRQA）** 基准，包含ARC-E/C、SIQA、PIQA、GSM8K以及用于评估事实性的LLaMA-QS。所有数据经LLM重写并TTS转换。
...
**早期推理方法准确率-延迟权衡图**：
![准确率-延迟权衡曲线](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/dFVenZdVbX-1.png)
