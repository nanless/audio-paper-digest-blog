...
AudioTrust 本身不是一个ALLM模型，而是一个用于评估ALLM可信度的基准框架和数据集。其架构是一个系统化的评估流水线，主要包含以下组件：
...
- 评估指标：针对每个维度和子任务设计了具体指标。例如：公平性使用群体公平性得分Γ；幻觉使用基于GPT-4o/Qwen3评估的多维准确度分数（0-10）；安全使用防御成功率（DSR）；隐私使用拒绝率；鲁棒性使用GPT-4o/Qwen3评估分数和跨模态词错率（CM-WER）；认证使用冒名拒绝率（IRR）。
...
- 主要Benchmark与结果：
    ...
    - 隐私（表4）：模型对直接隐私泄露（如复述银行卡号）的防御较好，尤其加入隐私提示后，GPT-4o mini Audio拒绝率达100%。但对隐私推断泄露（如从声音推断年龄、种族）的防御极弱，平均拒绝率仅9.02%，即使加入隐私提示提升也有限（约12.12%）。这表明模型未能识别从副语言线索推断出的信息属于隐私。
...
- 关键实验结果图表：
    ![论文中的模型可信度雷达图](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/E823AY0taq-2.jpg)
