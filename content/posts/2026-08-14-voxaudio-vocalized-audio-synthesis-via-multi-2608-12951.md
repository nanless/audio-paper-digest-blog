...
*   创新性 (1.5/2)：[A_METHOD] 提出随机 chunk 边界的因果自回归流匹配，使流式粒度可任意选择；并将 NFT 适配到流式噪声快照，构建 VoxCorpus/VoxBench，形成方法、数据与评测的联合贡献。

*   技术严谨性 (1.2/1.5)：[A_METHOD][SCORING_SOURCE_13/28] AR-FM 的 chunk-wise 因果 mask、独立噪声水平与滑动窗口推理在噪声调度上对齐；NFT 显式 replay 流式异步噪声快照，方法逻辑闭环。账本中未见可归因于算法推导失效的明显证据。

*   实验充分性 (1.0/1.5)：[A_RESULTS][A_LIMITS][SCORING_SOURCE_28/28] 在 VoxBench、MECAT、AudioCaps 和 SEED-TTS 上给出多指标对比与消融，支撑语音可懂度与时序定位声明；但 MECAT 分布指标明显落后且仅以领域差异解释，跨域泛化证据不足。

*   清晰度 (0.8/1)：[A_SUMMARY] 论文按 AR-FM、架构、训练、推理和评估分层展开，表格和指标定义完整，整体可读性较好。

*   影响力 (1.1/1.5)：[A_SUMMARY] 该工作直接面向播客、配音和实时音景合成，统一了可流式、时序可控的 vocalized audio 任务，对语音/音乐/音频社区有较直接的应用与基准影响。

*   开源 (1.0/1.5)：[A_OPEN] 代码与 Demo 页面已给出，但模型权重未说明，VoxCorpus/VoxBench 仅承诺非商业许可发布，核心产物只开放了部分，故按固定锚点记为 1.0。

*   可复现性 (0.4/0.5)：[A_METHOD] 论文披露了 VAE、DiT 结构、两阶段训练、关键超参数、评估协议与推理配置，但数据构建和完整复现步骤仍有少量缺失，不能完全不扣。

*   工程/实践价值 (1.2/1.5)：[A_METHOD][SCORING_SOURCE_27/28] 系统支持 KV-cache 滑动窗口流式推理、可变时长和低延迟输出；实测默认 RTF 约 0.32、首包延迟可低至 0.15 s，工程落地价值明确。
