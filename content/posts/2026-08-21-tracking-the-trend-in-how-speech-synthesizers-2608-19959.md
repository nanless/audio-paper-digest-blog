...
![Figure 3: Human detection per speaker for full-spoof and partial-spoof recordings. The prefix "mix" marks one synthetically replaced sentence; rows are recording sources, columns are speaker initials, and the final column is the mean. Green indicates reliably detected samples and red deceptive ones.](https://arxiv.org/html/2608.19959v1/fig_heatmaps_combined.png)
...
6. 材料不可公开使整个研究的独立复核不可能，结论的政策权重与可验证性不匹配。
7. 性别比例 70 比 11 且未测量名人熟悉度，个体差异因素的解释力受限。
8. 检测器仅用固定阈值推理，未做分数级校准或按说话人特征分层，低估了检测器的潜在上限。
