...
骗子红利的量化苗头是这项研究容易被忽略的贡献：随着听者警觉性提高，他们越来越倾向把真实录音误判为伪造——这种信任侵蚀对新闻采访素材、司法语音证据与电话沟通的伤害不亚于伪造本身。研究还发现警告的作用有限：即便被明确告知材料中有深伪，参与者的 ElevenLabs 全合成检测 F1 仍只有 48%。这对依赖「提高公众警惕」作为主要缓解手段的安全策略是一记冷水：感知防线失守后，剩下的选项只有程序化验证、来源认证、水印与片段级自动检测的分层组合。
...
部分篡改的设计选择反映了真实的攻击经济学：伪造整段语音的成本与风险都高于只改一句话，而一句话足以把「同意」变成「拒绝」、把陈述变成反问。实验中替换句由 ChatGPT-4 基于原转写改写并保证语义反转，再经所选工具合成拼回原录音，后期做音量归一与降噪消除拼接痕迹。评测粒度是本研究的方法论特色：参与者不是对整条录音做单一判断，而是对四个句子片段逐一分类，这使得分析能够区分「整体误判」与「特定插入句漏检」两种本质不同的失败——ElevenLabs 条件下真实句 88.59% 判对而插入句仅 22.65% 被识破的模式，只有句级评测才能揭示。
...
质性编码揭示了检测策略的代际差异：低分组参与者依赖「声音像机器人」「感觉不对劲」这类整体印象，高分组则能点名音高异常、节奏不自然、情感平淡与音频伪影等具体线索。面对老工具时这种专业直觉回报丰厚——最高四分位组的 All OK 得分达到 0.89 到 0.94 而最低组只有 0.23 到 0.24；但面对 ElevenLabs 时两组差距收窄到 0.08 对 0.01，说明可利用的声学线索本身消失了，个体技能的差异失去了作用对象。这一发现对安全培训的含义是消极但重要的：教育能教会人们寻找什么线索，却无法在线索不存在时创造线索。
