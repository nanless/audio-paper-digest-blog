...
1.  音频语言模型（ALM）作为规划器：
    ...
    *   结构：初始化自预训练的Audio Flamingo 2模型。其核心是一个CLAP音频编码器，用于从原始音频中提取语义特征 `za`。该特征通过Adapter层（包含自注意力和门控交叉注意力层）注入到一个冻结的大语言模型（Qwen2.5-3B） 中。指令 `P` 被分词后作为LLM的文本上下文。
...
2.  潜在扩散模型（LDM）作为编辑器：
    ...
    *   结构：包含一个立体声音频VAE 和一个 基于Transformer的扩散模型（DiT）。VAE将音频波形压缩到低维潜在空间。DiT模型以文本编码器（FLAN-T5）生成的 `s_i` 的嵌入为条件，通过交叉注意力机制对噪声潜在表示进行去噪。
    *   数据流：上一步音频 `a_i-1` 的潜在表示 `z_i-1` 与随机噪声 `z'_i` 拼接 -> DiT模型（条件：`s_i` 的文本嵌入���时间步 `t`） -> 预测噪声 -> 迭代去噪 -> 得到编辑后的潜在表示 `z_i` -> VAE解码 -> 下一步的输入音频 `a_i`。
    *   关键设计：采用速度预测和CFG重缩放技巧提升生成质量。通过顺序执行多个原子操作实现复杂编辑。
...
*   训练数据：
    ...
    *   规模：合成240k对复杂声明式编辑数据（训练ALM），以及1M对单步原子操作数据（训练LDM）。评估使用AudioCaps测试集生成的2k对复杂数据和3k对单步数据。
    *   空间建模：使用17cm耳间距、心形指向性模式、PyRoomAcoustics模拟混响（RT60在0.3-1.2s）来渲染立体声。
*   损失函数：
    *   ALM：标准的自回归语言建模损失（负对数似然），见公式(3)。
    *   LDM：去噪扩散损失（均方误差），见公式(4)。
*   训练策略：
    *   ALM：初始化自AF2模型。冻结CLAP和LLM大部分参数，全参数微调Adapter层，并对LLM的跨注意力层应用LoRA（rank=16）。训练20 epochs，batch size 24，学习率1e-5，优化器AdamW。
    *   LDM：DiT架构，24个block，每层1024通道，16个注意力头，总参数约597M。训练500k迭代，batch size 256，学习率5e-5，带50k步线性warmup和余弦衰减。使用10%的文本替换为空字符串以启用无条件建模。
*   关键超参数：
