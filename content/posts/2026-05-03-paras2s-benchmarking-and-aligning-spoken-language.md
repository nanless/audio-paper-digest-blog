...
- Stage 1: PolyTone训练。 针对不同副语言属性（情感、讽刺、性别、年龄）分别微调声学分析器。关键创新是PolyTone训练策略：使用相同文本内容但不同说话风格的语音对进行训练，迫使模型从语音信号而非文本内容中学习风格特征。
...
- 阶段2：奖励模型蒸馏（Reward Model Distillation）。
  ...
  - 使用完整的自动评判器管道（Stage 2中使用模型预测标签，即表2中的O5配置）对所有查询-回应对打分。
  - 收集（查询语音，回应语音，分数）三元组数据，通过LoRA微调一个Qwen2.5-Omni作为奖励模型。该模型以语音为输入，直接输出分数。
...
2.  抗风格幻觉的多阶段自动评判器： 发现了端到端Audio LLM（如GPT-4o）在评分时会基于语音内容“幻觉”出风格标签的问题。提出通过PolyTone训练和分离建模（专用分析器+AudioReasoner生成语气描述+文本LLM打分），将风格分析与内容解耦，显著提升了评判与人类偏好的相关性。
3.  基于可扩展AI反馈的RL对齐框架（ParaS2SAlign）： 将上述评判器蒸馏为奖励模型，用于在大量无标签语音上通过GRPO算法优化模型。这避免了收集昂贵且难以规模化的成对（查询， 恰当回应）示范数据的瓶颈。
