...
![Figure 1. Robot-view examples from the real-world HRI corpus: (a) a single-participant interaction, (b) a two-participant interaction, and (c) a three-participant interac](https://arxiv.org/html/2608.18401v1/figures/robot_view_examples_top.png)
...
* 创新性 (1.1/2)：[A_METHOD] 真实药店、多人数与自由退出场景有价值，建模主要采用成熟 backbone 和简单后融合。

* 技术严谨性 (1.1/1.5)：[A_RIGOR] session 级防泄漏、30 折与标签可靠性较扎实；LLM/embedding 不同评测条件限制公平比较。

* 实验充分性 (1.2/1.5)：[A_RESULTS] 多模型、多模态、互补性和条件分层较全面，97 个个体仍是小样本。

* 清晰度 (0.9/1)：[A_CLARITY] 数据过滤、标签、模态和融合权重都可追踪，结论没有把第三方 rapport 等同于内心体验。

* 影响力 (0.9/1.5)：[A_IMPACT] 对真实公共空间 HRI 评价有启发，跨文化与自主机器人外推仍受限。

* 开源 (0.5/1.5)：[A_OPEN] 模型依赖公开，但现场音视频受隐私限制，数据和完整代码开放方式不明确；按锚点规则对应「明确肯定语境中的未来开放承诺」。。

* 可复现性 (0.3/0.5)：[A_REPRO] 划分和指标清楚；闭源 API 版本、人工说话人校正和现场数据不可得会阻碍复现。

* 工程/实践价值 (1.0/1.5)：[A_ENGINEERING] 简单预测融合取得最好总体结果，适合模块化系统；预处理自动化与成本仍未解决。
