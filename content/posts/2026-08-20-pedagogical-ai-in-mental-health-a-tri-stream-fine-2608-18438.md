...
* 创新性 (1.2/2)：监督层和 D-CUI 的问题选择有新意，底层 OpenFace/COVAREP/LLM 组件相对常规。

* 技术严谨性 (1.0/1.5)：分数公式、阈值和人工责任清楚；规则标签、合成训练数据与同源 PHQ-8 风险/症状使独立性受限。

* 实验充分性 (1.1/1.5)：有 held-out 测试、置信区间与专家评分，但只有 22 个测试 session、2 类技巧。

* 清晰度 (0.8/1)：四阶段架构和三级分流直观，读者容易区分筛查与临床判断。

* 影响力 (0.8/1.5)：切中督导资源短缺，若真实治疗验证成立价值高；当前证据不足以支撑临床部署。

* 开源 (0.5/1.5)：采用可本地部署骨干与公开基准，但完整代码、适配权重和派生标签未形成明确开放包。

* 可复现性 (0.3/0.5)：超参数、硬件、公式与数据划分详细，规则标注和合成问答细节仍影响复现。

* 工程/实践价值 (1.0/1.5)：单 T4、约 10 秒/session 的成本可接受；63.6% 风险准确率决定它只能做辅助排序。
