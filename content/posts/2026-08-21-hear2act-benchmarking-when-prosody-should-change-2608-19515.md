...
![Figure 1: Hear2Act overview. (1) Each scenario combines a surface request, prioritized hidden concerns, and candidates with verifiable satisfaction signatures.](https://arxiv.org/html/2608.19515v1/hear2act_framework_crop.png)
...
场景种子的人设维度设计保证了同形异需的覆盖：同样的「找便宜机票」请求，紧迫出差的商务用户、带老人旅行的家庭、预算受限的学生对「过夜飞行」「行程长度」「白天出行」三层关切的优先级完全不同。候选集的构造则把这种差异转化为可验证的选择题——八种满足模式各对应一个候选，加上安全备选与两个违反明示要求的干扰项。助手若不挖掘隐藏关切，最合理的推荐就是表面属性最优的那个错误答案；只有通过针对性提问揭开硬约束，才能区分出真正合适的选项。最终选择因此成为「信息获取质量」的直接读数，无需人工评判。
...
![Figure 2: Illustrative Hear2Act trajectory under Prosody-mediated feedback. Three representative candidates are shown from the full 11-candidate set.](https://arxiv.org/html/2608.19515v1/hear2act_example.png)
...
| 音频推断状态入文 | 39.6% | 接近真值参考 |
| 真值关切状态 | 40.7% | 参考上界 |
| Claude Opus 4.6：转写→加状态标签 | OptSat 37.0%→41.1%，首推 49.7%→79.1% | 文本模型对照 |
| Kimi K2.5：转写→加状态标签 | OptSat 27.7%→36.1% | 文本模型对照 |
...
动作构成分析揭示了正确关切信息如何改变对话形态：拿到轮级关切状态的助手显著增加引发式提问、减少过早推荐，且更擅长判断何时该停止搜索——这回答了一个比「准确率」更细的问题：韵律信息的价值不仅在于选对答案，还在于塑造更好的信息获取策略。文本模型的对照实验进一步给出了决策价值的上限标定：Claude Opus 4.6 拿到真值状态标签后首推命中率从 49.7% 跃升至 79.1%，Kimi K2.5 的最优解率从 27.7% 升至 36.1%——不同基座对相同信息的利用效率差异明显，说明中间表示之外，下游策略组件同样有优化空间。
...
语音渲染层的验证流程同样严谨：由于合成语音可能丢失或扭曲设计者预期的韵律对比，作者让 Qwen2.5-Omni-7B 在相同片段上执行感知任务并与人类表现对照，只有确认交际对比在渲染后仍然存活的片段才进入主评估，避免了把合成缺陷误计为模型失败。统计层面，每个条件格运行两次共九百六十个样本点，所有关键对比使用配对的场景级比较并报告 95% 置信区间；动作构成分析进一步区分了「问得多」与「问得对」，防止把冗长对话误判为有效信息获取。
...
* 创新性 (1.3/2)：[A_METHOD] 五要素齐备的统一协议在评测设计上有实质创新，隐藏关切加客观签名的方案解决了韵律任务依赖主观评判的老问题；扣分在于场景由模板与引擎生成，真实世界副语言表达的丰富性无法完全覆盖。五要素定位表清晰划定了与既有基准的差异，避免了概念重叠的争议。
...
* 清晰度 (0.7/1)：[A_CLARITY] 设计动机与核心对比讲得清楚，但条件命名繁多（反馈×访问×渲染器），正文多处需借助附录才能还原完整配置。条件矩阵的组合爆炸使正文难以自洽呈现全部配置，需频繁查阅附录。
...
8. 审稿人发现：语音渲染器的选型与参数未披露，不同 TTS 系统的韵律表达能力差异可能影响结论的普适性。
9. 审稿人发现：确定性用户引擎的披露规则虽严谨但完全可预测，真实用户会给出不一致或误导性回应，助手在这种噪声下的表现未被测试。
