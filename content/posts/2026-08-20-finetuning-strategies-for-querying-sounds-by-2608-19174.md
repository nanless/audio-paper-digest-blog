...
* 创新性 (1.3/2)：[A_METHOD] semi-hard class-aware negatives 与动态 loss weighting 是扎实的任务适配，但主要组件沿用成熟对比/三元组学习。

* 技术严谨性 (1.0/1.5)：[A_RIGOR] 训练配置披露具体；两条 submission 改动因素过多，无法从当前表格做严格组件归因。

* 实验充分性 (1.1/1.5)：[A_RESULTS] 有官方 dev 指标和强基线，但缺随机种子方差、受试者/模仿风格细分、独立 loss ablation 与正文中的完整 test 表。

* 清晰度 (0.8/1)：[A_CLARITY] 数据角色、两条路线与指标定义清楚。

* 影响力 (0.7/1.5)：[A_IMPACT] QbVI 对音效搜索和无障碍交互有明确应用，但任务仍较专门。

* 开源 (1.0/1.5)：[A_OPEN] 提供在线部署页面与 challenge 链接；正文没有给出代码仓库、模型权重或训练产物下载；按锚点规则对应「核心产物可公开获取」。。

* 可复现性 (0.3/0.5)：[A_REPRO] 增强、输入特征、优化器和 loss 参数较完整，数据可得性与最终评测细节仍依赖外部站点。

* 工程/实践价值 (1.1/1.5)：[A_ENGINEERING] 任务清楚、路线有对照且工程味足；但“获胜”缺少可核验指标，读者不能仅凭摘要判断领先幅度。
