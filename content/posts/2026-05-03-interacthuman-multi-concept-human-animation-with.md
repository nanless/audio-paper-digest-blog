...
3.  **局部音频条件注入（推理阶段）**：
    ...
    *   **迭代缓存策略**：为了解决“掩码未知”与“需要掩码注入”的循环依赖，在去噪的前10步禁用掩码引导的音频注入。从第11步开始，使用**上一步（k-1）预测并缓存的掩码**来指导当前步（k）的音频注入。这使得掩码预测能随生成内容的演化逐步精炼。
...
*   **训练数据**：
    ...
    *   **规模**：最终构建了包含**超过260万个三元组**的数据集，每个三元组包含原始视频、逐帧的身份掩码和文本描述。
...
*   **损失函数**：
    *   **总体损失**：结合了**Flow-Matching扩散损失**和用于掩码分类的**Focal Loss**。
    *   **权重**：两者权重相等（1:1）。
...
*   **训练硬件**：在**32张A800 GPU**上训练了**10,000步**。使用FSDP（全分片数据并行）进行微调。
*   **推理细节**：
...
![消融研究定性对比](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/rJilRU8D3c-0.png)
