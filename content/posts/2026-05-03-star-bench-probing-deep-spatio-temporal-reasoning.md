...
*   训练数据：本文为基准测试论文，不涉及自有模型训练数据。评估所用音频数据来源明确：基础感知任务音频为程序化合成（纯音、Pyroomacoustics模拟）；整体推理任务音频源自Clotho, FSD50K, STARSS23及互联网收集，并经过严格的四阶段筛选与标注流程。
*   损失函数：未说明（不适用）。
*   训练策略：未说明（不适用）。
*   关键超参数：不适用。但基准本身有关键设计参数：基础感知任务的刺激差异等级（如音高差从0到1200音分）、空间任务的角度划分（如方位角四象限）、距离区间（0-10米）等。
*   训练硬件：未说明（不适用）。
*   推理细节：所有问题为多项选择题，通过字符串匹配选项标签或全文计算准确率。为保证鲁棒性，对感知和空间任务采用循环评测（选项顺序轮换N次），对时间任务采用片段顺序扰动（3次），最终报告平均准确率（AA）和全对率（ACR）。
*   正则化或稳定训练技巧：不适用。
...
![模型在时空任务上的误差分析](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/Ts6j3GoZDE-5.jpg)
...
*   关键消融实验（时间推理）：
![时间推理任务的消融研究](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/Ts6j3GoZDE-8.png)
