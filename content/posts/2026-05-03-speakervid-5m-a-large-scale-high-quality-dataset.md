...
论文在VidChatBench基准上评估了提出的基线模型，主要包含两个设置：条件设置（Conditioned）（以GT ASR和视频描述为条件）和双人设置（Dyadic）（以发起者音视频为输入直接生成回应）。关键结果如下表所示：
