...
1. 最强贡献是端到端约束，而非孤立压缩 1 个声学模型。时长、声学、解码和 iSTFT 全部进入 MCU 图，最终输出是 22.05 kHz PCM；这避免了把计算最重的声码器留给外部主机，却仍宣称“嵌入式 TTS”。

2. 三学生拆分也具有结构价值。40 维接口把语言条件和波形解码解耦，513 幅度加 1026 相位坐标让帧域解码器与固定 iSTFT 配合。条件 VAE 的随机教师被蒸馏为确定性执行链，更适合定点、缓存规划和跨平台 bit-level 测试。

3. 可审计交付由模型 manifests、二进制 fixtures 与 golden vectors 构成。模型 manifests、精确二进制 fixtures、golden vectors、运行时端口与校验和使论文中的参数量和相关性可复核。报告还主动区分真实实板、WASM、QEMU 和推算结果。创新边界同样明确：质量较高的 Amy 版本没有完成 MCU 运行，最小 Kristin 链与教师仍有显著听感差距。

4. 真实板测与质量 Pareto 分开展示也是重要工程贡献。作者没有用桌面模拟速度替代 S3 计时，也没有把 1,450,000 参数 Amy 模型的高分嫁接到 560,000 参数 Kristin 部署上。这个诚实拆分让读者看清当前可同时得到的能力组合。

5. 固定 40 维接口还允许未来分别替换时长或解码模块，而不重写所有端口；它是 1 种软硬件契约。代价是接口容量可能限制教师知识传递，Kristin 学生的质量差距提示现有瓶颈尚未定位到具体模块。
