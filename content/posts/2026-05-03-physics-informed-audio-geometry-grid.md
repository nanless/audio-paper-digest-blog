...
2.  音频-几何表示网络（AuGeonet）：
    *   首先使用LNuDFT（见公式3-8）对每个通道的音频信号进行变换，得到非均匀频带的频谱`Xc`。LNuDFT的频率点`νk`是可学习的，通过累加正增量`ak`定义，并初始化为在中频密集分布（logit初始化），这鼓励模型关注对IPD（通道间相位差）信息最丰富的频段。
    *   然后计算相对相位特征：以离阵列中心最近的麦克风为参考通道，计算其他通道与它的广义互相关-相位变换（GCC-PHAT）特征（公式9），得到`XGCC`，强调相位差异。
    *   相对麦克风位置编码（rMPE）（公式10-12）：将每个麦克风的球坐标（距离、方位角、仰角）转换为相对于参考通道的坐标，并编码为正弦位置向量。rMPE直接反映了引起TDOA（到达时间差）的物理基础——麦克风间的相对位置。
    *   `XGCC`和rMPE被送入一个基于通道维多头自注意力（CW-MHSA） 和门控循环单元（GRU） 的网络（AuGeonet，结构类似图4），其中rMPE被注入到注意力层和卷积块中，为模型提供几何先验。最终输出音频-几何表示`A`。
3.  网格表示网络（Gridnet）：
...
-   训练策略：
    *   采用复杂度渐进训练（CGT）（附录A.9）：1）多阶段几何学习（MSGL）：先固定简单几何（四面体）训练10轮，再用动态几何但固定4通道训练10轮，最后用动态几何和可变通道数（4-12）训练280轮。2）深度监督课程学习（DSCL）：从宽波束目标逐渐细化到窄波束目标。
    *   优化器：Adam，梯度裁剪至1。
...
关键结论：所提方法在所有四个评估数据集（包括两个真实、两个合成，涵盖seen和unseen条件）上均取得最优性能。特别是在最具挑战性的未见几何（Eigenmike，32通道）和未见通道数（Dynamic-U，13-16通道）条件下，优势巨大。例如在Eigenmike上，MAE从基线最优的14.89°（Unet）大幅降至11.24°，而GI-DOAEnet等几何感知方法则完全失效（MAE>90°）。
