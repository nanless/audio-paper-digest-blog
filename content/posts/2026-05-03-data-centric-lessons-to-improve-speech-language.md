...
2.  **结构化合成数据构建**：提出两种利用纯文本语料构建语音训练数据的合成方法。**Krist**从知识丰富网站提取并重写文本后合成语音，**Quest**则利用LLM从文本中挖掘问题并生成带思维链的答案。两者混合使用能有效覆盖网络爬取数据稀缺的领域（如科技、教育），提升模型性能。
...
- **训练数据**：
    ...
    - **合成数据**：**Krist**数据集约460万小时，来源于知识丰富网站的HTML文档，经LLM提取重写后，使用melo-TTS合成语音（5种口音随机选择）。**Quest**数据集约90万小时，同样来源于HTML文档，但专门构建为问答格式，由LLM（gpt-4o）生成带思维链的答案。
