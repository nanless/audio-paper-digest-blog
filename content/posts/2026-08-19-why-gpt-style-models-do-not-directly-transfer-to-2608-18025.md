...
理论框架的核心观察是：GPT 式下一 token 预测隐含假设了 token 序列的最优切分粒度，而音乐中「什么构成一个有意义的 token」取决于表示坐标系。受控实验通过固定学习者、事件空间和训练预算只变换表示坐标系来隔离因果变量。框架目前更像一套设计判据而非可直接部署的方法，这一差距需要更多真实音乐数据的验证来弥合。
...
此外，5 Experiments and Discussion 5.1 Protocol and Evaluation All matched comparisons preserve the learner, observable event space, and within-family training budget while changing only the representation interface.
