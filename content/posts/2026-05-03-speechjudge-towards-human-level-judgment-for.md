...
3.  RL阶段（强化学习）：使用SFT阶段处理后剩余的、教师模型判断错误的“挑战性”样本（图中“错误”部分）。此时，策略模型就是SFT后的模型。模型在CoT提示下生成多个输出（rollouts），每个输出会被解析出最终的自然度偏好判断（y_M）。将人类标注（y_H）作为可验证的奖励信号：如果判断一致（y_M = y_H），奖励为+1，否则为-1。使用GRPO（Group Relative Policy Optimization） 算法进行强化学习，目的是在只约束最终判断结果对齐人类偏好的前提下，让模型自主优化其推理和生成CoT的能力。
4.  推理时：最终训练好的GRM可以接收新的 `(t, a1, a2)` 对和CoT提示，输出一个包含详细推理过程的自然度判断。此外，它还支持推理时扩展：对于同一个输入，生成多个输出（如10个），然后通过多数投票决定最终结果，以提高准确率。

该模型本质上是一个判别模型被建模为生成任务：它不是直接输出一个分数，而是通过生成文本来表达其判断和推理过程，这使得CoT和投票机制成为可能。
...
- 训练数据：
    ...
    - SFT数据：在42K训练集中，使用Gemini-2.5-Flash生成CoT输出，选取其判断与人类一致的25K个样本。
    - RL数据：选取Gemini-2.5-Flash判断与人类不一致的17K个样本，仅使用其CoT提示（Prompt），由策略模型生成输出并计算奖励。
- 损失函数：
    - SFT阶段：标准的自回归语言建模损失（负对数似然），但仅在模型生成的输出部分（O_teacher） 进行计算。
    - RL阶段：使用GRPO算法。奖励函数是基于准确率的规则奖励：对于每个rollout，如果模型解析出的自然偏好（y_M）与人类标注（y_H）一致，奖励为1，否则为-1。优化目标是最大化期望奖励。
- 训练策略：
    - 优化器：AdamW（带解耦权重衰减）。
    - 学习率：SFT为5e-5；RL为5e-6。
    - 批量大小：SFT最大token数为4000/批；RL的batch size为32，每个prompt的rollout数为8。
    - 参数高效微调：两阶段均使用LoRA。SFT时秩（rank）为128；RL时秩为64。
    - 训练时长：未明确说明。
- 关键超参数：模型基座为Qwen2.5-Omni-7B。推理时扩展的投票数为10（Voting@10）。
- 训练硬件：未说明。
