...
categories: [论文速递]
description: "Vividh-ASR: A Complexity-Tiered Benchmark and Optimization Dynamics for Robust Indic Speech Recognition"
...
前50% | #语音识别 | #课程学习 | #迁移学习 #多语言 | [arxiv](https://arxiv.org/abs/2605.13087v1)

学术质量 5.8   （创新性2.0 + 技术严谨性1.5 + 实验充分性1.5 + 清晰度0.8）/8 | 影响力 0.7/2 | 可复现性 0.6/1 | 置信度 高
...
1. 整体流程概述：
这是一个针对Whisper等预训练语音识别模型进行微调的多阶段训练框架。其核心输入是按声学复杂度分层的语音数据集（Vividh-ASR），输出是微调后的模型。系统首先通过因子设计实验诊断影响微调效果的关键因素（学习率时序与课程顺序），然后据此设计一个三阶段的微调流水线（R-MFT），最后通过表示学习分析（CKA, SVD）来验证和解释微调过程中模型内部的变化。
