...
从信息流的角度补充：连续自回归表征的训练目标要求模型在声学保真与语义可解码之间取得平衡，语义教师蒸馏的梯度信号在这一权衡中起主导作用；Base 与 Instruct 双变体共享编码器但分化指令头，使同一表征同时服务克隆与编辑两类任务。此外，双变体的训练数据构成与采样策略也会影响表征的偏向性，这些细节对复现至关重要。
