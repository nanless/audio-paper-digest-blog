...
SongEcho基于预训练的文本到歌曲模型**ACE-Step**（一个Diffusion Transformer, DiT）构建。整体架构如图2所示。主要组件和数据流如下：

1.  **输入处理**：
    *   **歌词**：通过歌词编码器处理。
    *   **文本提示（标签）**：通过mT5编码器处理。
    *   **时间步**：通过时间嵌入器处理。
    *   **旋律条件**：输入的原曲声乐音高序列`p`（通过RVMPE在100Hz提取）经过一个**旋律编码器E**（包含1D卷积层）进行编码，得到特征`m'`，再插值到与隐藏状态`h_i`对齐的序列长度，得到`m`。

2.  **生成骨干网络**：
    *   使用ACE-Step的**线性Diffusion Transformer（DiT）**作为骨干。其Transformer块通常包含自注意力层、前馈网络（FFN）等。论文中，**IA-EiLM模块被集成到每个Transformer块中，位于FFN层之前**（如图2所示）。这种设计是为了避免自注意力层进行全局信息交互时稀释注入的旋律特征。
...
4.  **输出与训练**：
    ...
    *   训练时，仅训练**旋律编码器E**和**IA-EiLM模块**的参数，ACE-Step的主体参数保持冻结。
...
- **训练数据**：
    ...
    - 预处理：包括元数据完整性过滤、去除纯器乐和非英文歌曲、时长截断（≤4分钟）。使用**SongEval**进行五维质量评估并过滤低分（<3分）样本。
    - 数据增强：使用**Qwen2-audio**为歌曲生成全面的增强标签（流派、人声类型、乐器、情绪），与原标签合并去重，每首歌最多20个标签。
...
- **正则化或稳定训练技巧**：在EiLM中采用**零初始化**（公式11），确保训练开始时调制效果为零，从原始模型状态开始微调，避免初始噪声调制。
...
*   **数据规模消融**：仅用1000个样本训练即可达到接近完整数据集的效果，体现了方法的**数据高效性**。
