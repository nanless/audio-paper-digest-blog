...
4.  双输出头与联合监督：
    ...
    - 关键交互：在CTC驱动模式下，解码器的输入是教师CTC头输出的伪标签序列（经合并去重）。学生解码器被训练同时预测教师注意力PL和CTC PL。这建立了CTC输出与注意力解码器之间的直接耦合，是传递鲁棒性的关键。

### 💡 核心创新点

1.  CTC驱动的教师强制（伪标记生成）：
    - 局限：传统自回归解码速度慢，且错误会累积。
    - 作用：教师模型使用CTC头快速生成伪标签，作为固定输入条件喂给教师解码器，单次前向传播生成全部注意力PL。
...
2.  耦合的CTC-注意力联合预测（学生训练）：
    ...
    - 收益：使解码器从CTC PL中继承了单调对齐的鲁棒性，同时从注意力PL中学习序列建模能力。实验证明这对提升OOD性能至关重要（表4）。
3.  混合采样训练策略：
