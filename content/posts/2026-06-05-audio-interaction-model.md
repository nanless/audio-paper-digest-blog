...
*   实验充分性 (1.4/1.5)：在8个基准上进行了广泛评估，并进行了关键的消融研究（FIFO推理、流式训练、TFJP、分层事件选择、块大小、损失权重）。但主动响应基准`Proactive-Sound-Bench`的评估指标（平均准确率）过于简单，未评估响应质量、时机或用户接受度。与最强基线（如Qwen2.5-Omni 7B）的对比有待加强。
