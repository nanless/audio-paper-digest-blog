...
学术质量 5.6/7 | 影响力 1.7/2 | 可复现性 1.1/2 | 置信度 高
...
*   **影响力：1.7/2**
    论文对**音乐生成**领域有明确且重要的影响：（1）在公开挑战赛中取得了顶尖性能，验证了其训练策略的有效性；（2）提出的“训练时锚点”假说为未来设计和使用条件扩散模型（尤其是带有辅助编码器的架构）提供了重要的实证依据。影响力因基于特定挑战赛数据集（457小时）且与使用更大数据集训练的外部基线对比时，性能优势可能部分归因于数据量差异而略有局限。
