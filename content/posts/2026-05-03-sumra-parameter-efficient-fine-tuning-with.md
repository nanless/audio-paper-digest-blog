...
3.  **架构图说明**：
    ...
    **图1**：对比了全量微调、标准LoRA、PiSSA和SumRA。关键区别在于右下角：SumRA通过**求和多个奇异向量**来初始化A（橙色模块），并冻结A，仅训练B（橙色模块）。这直观展示了SumRA如何将更广的知识压缩到A中，并通过共享A节省存储。

    ![论文中的图片](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/v23Pqcm6qp-1.png)
    **图2**：详细解释了A矩阵的初始化策略。D)子图展示了SumRA的核心思想：将`Σ^(1/2)V⊤`的多行**求和**后赋给A的一行，使这一行能同时影响对应于多个奇异向量的知识概念，从而扩大影响范围。

    ![论文中的图片](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/v23Pqcm6qp-2.png)
    **图3**：展示了不同的求和策略。A)块求和会导致重要向量集中，产生干扰；B)交错求和和C)贪婪求和则旨在将重要向量**均匀分配**到A的不同行，以平衡负载（`L_j`），减少干扰。

### 💡 核心创新点

1.  **奇异向量求和初始化**：这是最核心的创新。认识到仅用主要奇异向量（PiSSA）会限制A的表征范围，提出将**多个（可能是全部）** 奇异向量求和后初始化A的每一行。这使A能在更全局的知识空间上进行适应，尤其适用于需要广泛调整的任务（如口音、风格）。
2.  **知识负载均衡的求和策略**：为防止求和时重要向量相互干扰，提出了“交错求和”和“贪婪求和”策略。后者被证明在数学上能最优地最小化“行最大负载”（`L_max`），确保重要知识分布均匀，提升了初始化的质量。
3.  **参数高效的存储共享**：在改进初始化的同时，继承并强化了LoRA-FA的思路（冻结A，只训练B）。通过使A的构造仅依赖于预训练权重 `W0`（与任务无关），实现了**跨任务的A矩阵共享**。这在多任务、多语言场景下能将额外存储成本降低一半以上（只存B）。
...
**主要对比实验结果（表2）：**
在Whisper-large-v2（r=32）上，SumRA以17.6M的额外参数（仅为LoRA/PiSSA的约一半），在所有五种语言上均取得了最佳WER。相比最强基线CorDA，SumRA平均降低约8.6%的相对WER；相比标准LoRA，平均降低约12.2%的相对WER。这证明了其在**性能-效率**权衡上的优越性。
...
**不同数据规模影响（表4）：**
在Esperanto语言上，随着训练数据从10h增加到100h，SumRA（冻结A）相对于LoRA的优势逐渐缩小（10h时相对提升11.2%，100h时接近持平），表明SumRA在**低资源**场景下优势最为明显。
...
- **开源与复现加成 (0.0/1)**：论文提供了详细的实验协议和结果，部分支持复现。但**完全未提供代码、模型权重或具体超参数（如学习率）**，也未提及任何开源计划，这大大增加了完全独立复现的难度。
