...
1.  模拟用户的局限性：论文承认用户是模拟的，而非真实人类。模拟用户的行为（尤其是两阶段意图分配逻辑）直接决定了Proc分数的计算。尽管有审计，但模拟用户与真实用户在揭示隐藏意图的时机、方式和丰富度上可能存在系统性差异，这影响了结果的生态效度。
2.  评估协议的潜在偏差：Proc和Comp的计算高度依赖GPT-5.4作为“法官”。这可能导致评估结果反映的是与GPT-5.4偏好或判断模式对齐的程度，而非绝对的主动性或任务完成能力。虽然进行了审计，但审计本身也使用了类似的模型。
3.  隐藏意图定义的挑战：意图的“推断”与“提供”之间的界限有时模糊。用户代理对“聚焦提问”的判定标准可能过于严格或宽松，这直接影响Proc的计算。论文中低比例的“已推断”状态可能反映了当前模型提问能力弱，但也可能反映了评估标准不易被触发。
4.  基准任务的覆盖范围：尽管覆盖了5个领域，但所有任务都在特定的、预设的模拟应用环境中完成。现实世界中的个人助手可能需要处理更开放、更动态、与真实网络和设备更深度集成的任务，这些场景在π-Bench中未被充分代表。
5.  对“主动性”的定义可能过于狭窄：本文将主动性主要定义为“提前发现隐藏意图”。但真实的主动助手可能还包括：在用户未察觉时主动提出优化建议、主动监控并预警潜在问题、主动学习用户习惯等。π-Bench未涵盖这些更广泛的主动性形式。
6.  实验分析的深度局限：论文展示了Proc和Comp的负相关趋势，但未深入探讨造成这种差异的模型内在原因（例如，是推理能力、规划能力还是工具使用能力导致了这种差异？）。论文主要呈现了现象，而非深入的因果分析。
7.  跨领域泛化的不确定性：该基准在特定的专业领域任务（法律、金融、医药）上构建，其评估结论能否泛化到其他领域（如日常生活、娱乐）的个人助手场景，尚不清楚。
