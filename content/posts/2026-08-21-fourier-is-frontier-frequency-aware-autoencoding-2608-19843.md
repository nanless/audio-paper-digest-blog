...
谱前端的具体口径：nfft=960 对应 50Hz 频率间隔，hop=480 即 100Hz 帧率，单侧谱 481 个 bin 去除奈奎斯特后保留 480 个；中间层逐步对频率轴下采样，F_ℓ 记录各层频率位数。Spec-SnakeBeta 的数值细节包括 1e-9 的分母稳定项与 1e-6 的初始化下限，对数参数化带来链式法则的梯度缩放恒等式，作者特别说明这只是实现性质而非收敛性声明。判别器的多尺度配置为 FFT 128 到 2048 五个二倍尺寸（hop 减半保持 50% 重叠）加固定 4096 配三种 hop（2048、1024、256），后者在 48kHz 下提供约 11.7Hz 的频率分辨率与三种时间密度；每个子判别器接收实部、虚部与幅值三路输入。评价指标覆盖 SI-SDR、四种谱距离（线性与 log1p 版本的 STFT 和 Mel 幅度 L1 距离）、CCPC 跨声道相位一致性与 SPE 强度声像误差；度量用的高频段（8kHz 以上）与 Refiner 的修正带（1.5/4kHz 分界）是两套独立定义。探索性的隐空间探针发现 ear-VAE2 的时间-频率「反转」比最大，即快速变化的隐分量解码出较低谱质心的音频，作者将其标注为诊断性观察而非主张。未披露信息包括：三阶段训练各自的步数与批大小、专家听音测试的人数与刺激数量、代码或权重发布计划。再补充两处实现层面的观察：其一，损失计算全程使用 float64 精度，这在复数域运算里并非奢侈而是必需，实部虚部与幅值混合的梯度路径对数值误差敏感，工程复现时容易踩坑；其二，判别器输入统一为实部、虚部与幅值三通道拼接，谱路径判别器直接作用于精修后的复数输出而非重合成波形，两条监督路径的分工使对抗信号既覆盖感知层又覆盖谱结构层。数据侧的自动化质量流水线细节放在附录，正文只披露了规模与来源构成，过滤标准的具体阈值未公开。

判别器配置的细节反映了多尺度感知的设计意图：五个二倍 FFT 尺寸（128 到 2048）保证时间分辨率从粗到细的覆盖，三个固定 4096 FFT 配不同 hop 的子判别器在约 11.7Hz 频率分辨率下提供三种时间密度，专门捕捉和声结构的细微差别；每个子判别器接收实部、虚部与幅值三路输入。损失体系融合了 K 加权响度、中侧左右立体声分解、自适应对数幅度归一化与混合尺度谱损失等多种感知技巧，且复数域计算全程使用 float64 精度——这些工程细节在复现时容易踩坑，论文逐一写明是负责任的。探索性的隐空间探针发现该模型的时间-频率反转比最大，即快速变化的隐分量解码出较低谱质心的音频，作者将其谨慎标注为诊断性观察而非主张。
...
3. 审稿人发现：训练数据含一万小时专有内部音乐，最优结果的数据基础对外部研究者不可复制，开源缺失进一步放大了这一壁垒。
4. 审稿人发现：专家配对评分未报告评审人数、刺激数量与显著性检验，主观优势的证据强度难以评估。
5. 审稿人发现：Refiner 只在冻结解码器输出上训练，若解码器继续更新，分带修正的最优性可能失效，论文未讨论联合训练或迭代精修的兼容性。
6. 审稿人发现：双工理论分带边界（1.5 与 4kHz）为固定常数，未探索边界位置或带数的敏感性。
7. 审稿人发现：演示页之外没有任何代码、权重或训练配置的发布承诺，与论文的受控严谨主张形成落差。
