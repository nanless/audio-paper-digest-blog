...
2.  细粒度评分投影：将编码表示 `h` 输入到一个序数离散化层（Ordinal Discretization Layer, ODL）。ODL 将 `h` 映射到 `K` 个维度（对应18个人类相似性维度）的潜在分数 `z`。为尊重人类评分的序数性质（1-5分），ODL 使用累计链接函数，通过可学习的尺度参数和有序切分点，将每个维度的潜在分数转换为一个关于有序类别的概率分布。通过最小化在所有样本和维度上的序数负对数似然来训练ODL，使其输出与人类评分对齐的、序数保持的可解释分数。
3.  可解释二元分类：将ODL输出的可解释分数 `z` 输入到一个带正则化约束的线性分类器中。该分类器的权重矩阵 `W_F` 被正则化，以确保最终的人机二元分类决策保持可解释性。目标函数是交叉熵损失与权重对称正则化项的加权和。
...
- 损失函数：
    - ODL训练：最小化序数负对数似然，以学习预测每个维度的正确评分等级。
    - 线性分类器训练：交叉熵损失，并加入权重矩阵的对称正则化项，权重 `λ=0.1`。
- 训练策略：
    - 优化器：Adam。
...
参与者识别准确率（见图4b）：
图4b显示，参与者区分人机的能力与AI熟悉度高度相关（最熟悉组78.8% vs. 最不熟悉组64.2%），而与性别、教育水平关系不大。
