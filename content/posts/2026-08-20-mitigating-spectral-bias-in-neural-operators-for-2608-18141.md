...
* 实验充分性 (1.1/1.5)：在 South China Sea 数据上，论文报告 S2RL 显著优于 FNO 基线，并保持毫秒级推理；摘要未给出具体误差、速度和不同频段表格，因此不补写数值。；未披露的数字、基线或细分实验保持未知。
