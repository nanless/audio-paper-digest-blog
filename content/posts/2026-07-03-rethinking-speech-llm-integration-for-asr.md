...
- 训练策略：两阶段训练。阶段一仅更新适配器适配器参数，学习率1e-4，覆盖10% ASR token；阶段二全参数（编码器、适配器、LLM）联合优化，学习率4e-5。均使用AdamW优化器、线性衰减。采用8k上下文的数据打包策略，配合FlashAttention的`cu_seqlens`机制防止样本间信息泄露。
