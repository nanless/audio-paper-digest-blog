...
论文主要基于Qwen2.5-Omni-7B多模态大语言模型构建OMNISAPIENS-7B。整体架构如下：
...
4.  行为适配器模块（BAM）：这是OMNISAPIENS-7B BAM的核心创新。它在冻结的SFT模型基础上附加一个残差适配器。
...
消融实验（论文Table 6 & 10）
- BAM的有效性：完全移除原始音视频特征（BAM-ABL）后，性能在多数任务上弱于完整的BAM，证明行为描述符是补充性而非替代性信息。
