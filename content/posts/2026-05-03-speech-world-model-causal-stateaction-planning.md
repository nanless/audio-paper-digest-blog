...
1.  **输入编码与融合**：
    ...
    *   三者通过一个**融合模块**（`φ`）结合成一个统一特征向量 `g`。论文探索了门控融合（基线）、注意力融合和Transformer融合三种机制。
2.  **因果图推理**：
...
- **评估指标**：1) **节点质量**：分类准确率（Accuracy）和加权F1。2) **边缘因果效应**：平均因果效应（ACE）和干预一致性分数（ICS），用于量化模块间学习到的因果关系强度。3) **指令微调效果**：使用GPT-4o作为评委，对推理质量和回应质量进行0-10分的打分（综合分为0.6*推理分 + 0.4*回应分）。
