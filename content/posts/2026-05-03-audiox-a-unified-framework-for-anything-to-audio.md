...
3.  **多模态融合**：这是核心创新点。三个模态的嵌入向量被送入**多模态自适应融合（MAF）模块**（图4右侧）。MAF模块依次执行：
...
4.  **条件融合与生成**：将校准后的特征拼接，形成最终的多模态条件嵌入H_c。H_c与扩散时间步t一起，通过交叉注意力条件化**扩散Transformer（DiT）** 骨干网络。DiT处理加噪的潜在表示z_t，执行反向扩散过程，最终通过音频解码器输出生成的音频A。
