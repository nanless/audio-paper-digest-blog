...
MIAM本身是一个掩码生成策略，而非独立的预测模型。它被集成到一个标准的多模态Transformer架构中：
...
2.  掩码层：MIAM为每个模态的每个token生成一个掩码概率pm。根据公式(2)和(5)，从混合Beta乘积分布中采样出概率向量p = (p1, ..., pM)。然后，以对应模态的概率pm独立地对每个token进行掩码（即以pm的概率将其替换为可学习的`[MASK]`嵌入）。
...
- 训练数据：
    - GeoPlant：物种存在-缺失数据，包含三个模态：(1) 48个表格环境变量（如海拔、土壤属性）；(2) Sentinel-2卫星影像（4波段，分为25个5x5 patch）；(3) 气候时间序列（4变量，2000-2018年）和Landsat时间序列（6波段，2000-2020年）。数据划分为训练/验证/测试集（70%/15%/15%），采用1°×1°的空间块交叉验证以缓解空间自相关。保留观测数>20的物种，最终评估1783个物种。
...
- 损失函数：
    - GeoPlant：使用加权二元交叉熵损失（W-BCE），处理多标签分类任务中的类别不平衡。
    - TaxaBench：使用标准的交叉熵损失。
- 训练策略：
    - 优化器：AdamW，权重衰减0.01。
    - 学习率：0.001，采用无调度策略（schedule-free approach）。
    - Batch size：128。
    - Dropout率：0.1。
    - 训练轮数：100 epochs，使用早停（基于验证集上各单模态及全模态平均AUC/准确率）。
- 关键超参数：
