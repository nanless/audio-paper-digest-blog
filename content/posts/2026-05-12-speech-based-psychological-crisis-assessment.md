...
categories: [论文速递]
description: "Speech-based Psychological Crisis Assessment using LLMs"
...
前25% | #语音情感识别 | #大语言模型 | #数据增强 #多任务学习 | [arxiv](https://arxiv.org/abs/2605.10027v1)
...
1.  副语言注入模块：
    ...
    *   输入输出：输入为原始音频波形和基础ASR转录文本；输出为副语言增强后的转录文本。例如，输入`“I was forced to move out of the school dorm…”`，输出为`“I was forced to move out of the school dorm… [Trembling voice, obvious sobbing, low volume, expressing extreme sadness and helplessness; Emotion: Sadness >> Anxiety]’’`。
...
图1：数据统计展示了数据集的基本特征：(a) 通话时长分布呈双峰，长短对话都有；(b) 来电者年龄近似正态分布，峰值在22岁左右；(c) 三个危机等级（无、低、中高）的样本数相对均衡；(d) 显示危机等级越高的通话，其平均时长倾向于更长。这些统计信息为理解数据分布提供了基础。
...
*   正则化技巧：主要依赖数据增强和辅助损失（ℒ_gen） 作为正则化手段。论文未提及Dropout等其他明确的正则化技术。
