...
2.  **核心模型**：基于**扩散Transformer（DiT）**，参数量419M。
...
![SpeechOp 架构图](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/eLsEjjFODE-2.png)
...
2.  **任务组合分类器自由引导（TC-CFG）**：**最核心的理论创新**。基于贝叶斯分解和条件独立假设，将组合任务的得分函数分解为“任务特定得分”和“判别式引导项”。这允许在推理时，用一个任务（如增强）的得分函数保持其特定目标（声学质量），同时用另一个任务（如TTS）模型作为**判别器**（而非生成器）来提供内容引导，从而避免了混合生成先验导致的质量下降。
...
- **损失函数**：
    ...
    - 采用**速度参数化**（v = α_tε - σ_t x）以稳定训练。
    - 使用**Sigmoid损失加权**（偏置-2.5），聚焦于感知相关的噪声水平。
...
![语音增强与分离训练曲线](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/eLsEjjFODE-1.png)
...
![1D高斯混合模型模拟对比](/audio-paper-digest-blog/images/iclr-2026/2026-05-03/eLsEjjFODE-3.png)
