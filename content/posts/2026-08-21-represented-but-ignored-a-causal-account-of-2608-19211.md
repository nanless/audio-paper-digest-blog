...
![Figure 1: Probe ladder for localizing prosody failures in audio-LLMs. The ladder follows prosodic information from the audio pathway, through the LLM representation, to the final a](https://arxiv.org/html/2608.19211v1/x1.png)
...
![Figure 5: Two interventions at the lens-peak layer L∗L*. (A) Direction injection. A class-mean direction d=μ+−μ−d=+-- is computed from positive- and negative-class L](https://arxiv.org/html/2608.19211v1/x4.png)
...
![Figure 4: Behavioral ladders for the three main cells. Color identifies the model. Top to bottom within each block: Qwen2.5-Omni, Phi-4-MM, Audio-Flamingo-3,](https://arxiv.org/html/2608.19211v1/x3.png)
...
| Qwen×VESUS | 0.63 | 0.93±0.06 | 24% | 部分 F2 |
| Phi-4×VESUS | 0.56 | 0.62±0.06 | 16% | F1 |
...
![Figure 6: Acoustic correlates in sparse features. One example cell from each of three models, chosen to cover the three datasets.](https://arxiv.org/html/2608.19211v1/x5.png)

情绪单元的顶部归因特征与 RMS 能量变化呈强相关，符合已知的声音唤醒线索；语调单元与 F0 斜率的对齐则弱得多——特征可解释性在不同韵律维度上并不均匀，作者对此的限定是诚实的。

干预实验的量化细节值得一提：十八个干净单元的方向注入斜率全部为正且置信区间排除零，范围从 0.29 到 3.20 个对数几率单位；α 扫描显示十五个单元在测试范围内召回饱和，例如 Qwen 与 CREMA-D 的中性/愤怒单元愤怒召回从 α 为零时的 0.366 升至 α 为二时的 1.000。但恢复是方向性的而非选择性的——推得过远时负类样本同样跨越边界，这说明编辑的是一条粗粒度的偏置通路，距离精准修复还有本质差距。稀疏空间的复制实验则给出了更精细的控制粒度：覆盖九成五归因的特征集仅含三十三到一百零二个坐标，钳制它们即可在十七八个单元中恢复被抑制类别。

### ⚖️ 评分理由

* 创新性 (1.5/2)：[A_METHOD] 失效分类学加探针阶梯的诊断框架是音频-LLM 可解释性方向缺失已久的基础设施，把文本域成熟的因果干预工具首次系统性地用于跨模态失效定位，问题形式化本身即贡献；扣分在于单项技术均为既有工具的组合应用。F1/F2/F3 分类学预计会成为音频-LLM 失效分析的标准词汇。
...
* 工程/实践价值 (0.6/1.5)：[A_ENGINEERING] 探针阶梯可作为模型诊断工具箱直接复用，干预结果表明解码端微调或提示工程可能比换音频编码器更有效；但方向性恢复的非选择性限制了直接落地。诊断阶梯可直接嵌入模型迭代流程作为回归测试。
